                                                ...
                                                VkBuffer pointBuffer)
{
  ...
  if(!pipelineMatches)
  {
    ...
    m_colorFormat = previewPipeline.getColorFormat();
    m_depthFormat = previewPipeline.getDepthAttachmentFormat();
    createRenderPass(previewPipeline);
...
void LidarPointOverlayPipeline::draw(const VkCommandBuffer& cmdBuf,
                                     ...
                                     uint32_t pointCount)
{
  if(!config.enabled || pointCount == 0 || config.pointSizePixels <= 0.0f || m_renderPass == VK_NULL_HANDLE ||
  ...
  if(m_debug != nullptr)
  {
    m_debug->beginLabel(cmdBuf, "LidarPointOverlay");
  }
  ...
  PushConstantLidarOverlay pushConstant{};
  pushConstant.sensorIndex = config.sensorIndex;
  pushConstant.pointSizePixels = config.pointSizePixels;
  vkCmdPushConstants(cmdBuf, m_pipelineLayout, VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_FRAGMENT_BIT, 0,
                     sizeof(pushConstant), &pushConstant);
  vkCmdDraw(cmdBuf, pointCount, 1, 0, 0);
...
void LidarPointOverlayPipeline::createDescriptorResources()
{
  m_bindings.clear();
...
void LidarPointOverlayPipeline::updateDescriptorSet(VkBuffer sensorBuffer, VkBuffer pointBuffer)
{
...
void LidarPointOverlayPipeline::createRenderPass(const PreviewRasterPipeline&)
{
  ...
  {
    throw std::runtime_error("Failed to create LiDAR overlay render pass");
  }
}

void LidarPointOverlayPipeline::createFramebuffer(const PreviewRasterPipeline& previewPipeline)
{
  destroyFramebuffer();
  ...
  {
    throw std::runtime_error("Failed to create LiDAR overlay framebuffer");
  }
}

void LidarPointOverlayPipeline::createGraphicsPipeline()
{
  ...
  VkPushConstantRange pushConstant{VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_FRAGMENT_BIT, 0,
                                   sizeof(PushConstantLidarOverlay)};
  ...
  {
    throw std::runtime_error("Failed to create LiDAR overlay pipeline layout");
  }
  ...
  pipelineGenerator.addShader(nvh::loadFile("spv/lidar_overlay.vert.spv", true, defaultSearchPaths, true),
                              VK_SHADER_STAGE_VERTEX_BIT);
  pipelineGenerator.addShader(nvh::loadFile("spv/lidar_overlay.frag.spv", true, defaultSearchPaths, true),
                              VK_SHADER_STAGE_FRAGMENT_BIT);
  m_pipeline = pipelineGenerator.createPipeline();
  if(m_pipeline == VK_NULL_HANDLE)
  {
    throw std::runtime_error("Failed to create LiDAR overlay graphics pipeline");
  }
}

void LidarPointOverlayPipeline::destroyFramebuffer()
{
