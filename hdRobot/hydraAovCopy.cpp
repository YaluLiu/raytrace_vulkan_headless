...
bool CheckGlError(const char *operation)
{
  ...
  std::cerr << "[HydraAovCopy] " << operation << " failed with GL error 0x" << std::hex << error << std::dec
            << std::endl;
  return false;
}
                           ...
                           ::HdRobotGlInteropCache &glInteropCache)
{
  ...
  {
    std::cerr << "[HydraAovCopy] Missing render buffer for AOV " << name.GetString() << std::endl;
    return false;
  }

  const std::optional<ExportedAovTexture> src = app.GetAovTexture(request.engineAov);
  if(!src)
  {
    std::cerr << "[HydraAovCopy] Missing source AOV texture for " << name.GetString() << std::endl;
    return false;
  }
  ...
  if(width <= 0 || height <= 0)
  {
    std::cerr << "[HydraAovCopy] Invalid render buffer size for AOV " << name.GetString() << ": " << width << "x"
              << height << std::endl;
    return false;
  }
  ...
  if(dstTextureId == 0)
  {
    std::cerr << "[HydraAovCopy] Invalid destination GL texture id for AOV " << name.GetString() << std::endl;
    return false;
  }
  ...
  auto copyFromSourceTexture = [&](GLuint srcTextureId)
  {
    ...
    {
      std::cerr << "[HydraAovCopy] Invalid source GL texture id for AOV " << name.GetString() << " (src="
    ...
    {
      std::cerr << "[HydraAovCopy] Invalid source AOV size for " << name.GetString() << ": " << srcWidth << "x"
                << srcHeight << std::endl;
      return false;
    }
    ...
    {
      std::cerr << "[HydraAovCopy] Failed to create blit FBOs for AOV " << name.GetString() << std::endl;
    ...
    {
      std::cerr << "[HydraAovCopy] Incomplete blit FBO for AOV " << name.GetString() << " (read=0x" << std::hex
