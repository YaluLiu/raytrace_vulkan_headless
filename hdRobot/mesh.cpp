...
void _CalculateFallbackTangents(const VtVec3fArray& meshNormals, VtVec3fArray& meshTangents, VtFloatArray& meshBitangentSigns)
{
...
VtValue _ExpandBufferElements(const HdVtBufferSource& buffer, HdType type, size_t elementExpansion)
{
  ...
  else if(type == HdTypeInt32Vec4)
    return _ExpandBufferElements<GfVec4i>(buffer, elementExpansion);
  else if(type == HdTypeInt32Vec3)
    return _ExpandBufferElements<GfVec3i>(buffer, elementExpansion);
  else if(type == HdTypeInt32Vec2)
    return _ExpandBufferElements<GfVec2i>(buffer, elementExpansion);
  else if(type == HdTypeInt32)
    return _ExpandBufferElements<int32_t>(buffer, elementExpansion);
...
{
  if(type == HdTypeFloatVec4)
    return _DeindexBufferElements<GfVec4f>(buffer, faces);
  else if(type == HdTypeFloatVec3)
    return _DeindexBufferElements<GfVec3f>(buffer, faces);
  else if(type == HdTypeFloatVec2)
    return _DeindexBufferElements<GfVec2f>(buffer, faces);
  else if(type == HdTypeFloat)
    return _DeindexBufferElements<float>(buffer, faces);
  else if(type == HdTypeInt32Vec4)
    return _DeindexBufferElements<GfVec4i>(buffer, faces);
  else if(type == HdTypeInt32Vec3)
    return _DeindexBufferElements<GfVec3i>(buffer, faces);
  else if(type == HdTypeInt32Vec2)
    return _DeindexBufferElements<GfVec2i>(buffer, faces);
  else if(type == HdTypeInt32)
    return _DeindexBufferElements<int32_t>(buffer, faces);
...
{
  if(type == HdTypeFloatVec4)
    return VtValue(VtVec4fArray(elementCount));
  else if(type == HdTypeFloatVec3)
    return VtValue(VtVec3fArray(elementCount));
  else if(type == HdTypeFloatVec2)
    return VtValue(VtVec2fArray(elementCount));
  else if(type == HdTypeFloat)
    return VtValue(VtFloatArray(elementCount));
  else if(type == HdTypeInt32Vec4)
    return VtValue(VtVec4iArray(elementCount));
  else if(type == HdTypeInt32Vec3)
    return VtValue(VtVec3iArray(elementCount));
  else if(type == HdTypeInt32Vec2)
    return VtValue(VtVec2iArray(elementCount));
  else if(type == HdTypeInt32)
    return VtValue(VtIntArray(elementCount));

  TF_AXIOM(false);
  return VtValue();
}

const static TfToken _texcoordPrimvarNameHints[] = {UsdUtilsGetPrimaryUVSetName(),
                                                    _tokens->st,
                                                    _tokens->st0,
                                                    _tokens->st_0,
                                                    _tokens->st1,
...
{
  _meshData.scene_mat_ids = {_sceneStore.ResolveMaterialIndexForUpload(_sceneStore.GetDefaultMaterialHandle())};
}
...
void HdRobotMesh::Finalize(HdRenderParam* renderParam)
{
  ...
  _sceneStore.DestroyMesh(_meshHandle);
}
                                            ...
                                            const HydraMaterial* sourceMaterial, HydraMesh& meshData)
{
  ...
  curMatId = _sceneStore.ResolveMaterialIndexForUpload(_displayColorMaterialHandle);
    ...
    if(materialPrim)
    {
      if(materialPrim->GetBaseColorPrimvarName().IsEmpty()
         || !ApplyDisplayColorMaterial(sceneDelegate,
                                       ...
                                       meshData))
      {
        const int materialIndex = _sceneStore.ResolveMaterialIndexForUpload(materialPrim->GetHandle());
        if(meshData.scene_mat_ids.empty())
        {
                                                                              ...
                                                                              bool forceVertexInterpolation)
{
  ...
  return ProcessedPrimvar{.interpolation = forceVertexInterpolation ? HdInterpolationVertex : primvarDesc.interpolation,
                          .type          = type,
                                                          ...
                                                          bool                indexingAllowed)
{
  ...
  for(size_t threadIdx = 0; threadIdx < numThreads; ++threadIdx)
  {
    ...
    futures.emplace_back(std::async(std::launch::async,
                                    [this, sceneDelegate, &primitiveParams, &faces, vertexCount, indexingAllowed, &validPrimvars,
                                     startIdx, endIdx]() -> std::vector<std::pair<TfToken, ProcessedPrimvar>> {
...
bool HdRobotMesh::_CreateGiMeshes(HdSceneDelegate* sceneDelegate, HydraMesh& meshData)
{
  ...
  const int defaultSceneMatId = meshData.scene_mat_ids.empty()
                                    ? _sceneStore.ResolveMaterialIndexForUpload(
                                          _sceneStore.GetDefaultMaterialHandle())
  ...
  for(size_t i = 0; i < geomSubsets.size(); i++)
  {
    ...
    if(materialPrim)
    {
      meshData.scene_mat_ids.emplace_back(_sceneStore.ResolveMaterialIndexForUpload(materialPrim->GetHandle()));
