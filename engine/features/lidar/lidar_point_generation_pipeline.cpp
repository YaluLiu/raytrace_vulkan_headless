                                            ...
                                            uint32_t height)
{
  ...
  if(m_debug != nullptr)
  {
    m_debug->beginLabel(cmdBuf, "LidarPointGenerate");
  }
  ...
  vkCmdPushConstants(cmdBuf, m_pipelineLayout, VK_SHADER_STAGE_COMPUTE_BIT, 0, sizeof(pushConstant), &pushConstant);
...
void LidarPointGenerationPipeline::createPipeline()
{
  vkDestroyPipeline(m_device, m_pipeline, nullptr);
  vkDestroyPipelineLayout(m_device, m_pipelineLayout, nullptr);

  VkPushConstantRange pushConstant{VK_SHADER_STAGE_COMPUTE_BIT, 0, sizeof(PushConstantLidarGenerate)};
  ...
  {
    throw std::runtime_error("Failed to create LiDAR point generation pipeline layout");
  }

  VkComputePipelineCreateInfo pipelineInfo{VK_STRUCTURE_TYPE_COMPUTE_PIPELINE_CREATE_INFO};
  pipelineInfo.layout = m_pipelineLayout;
  pipelineInfo.stage = nvvk::createShaderStageInfo(
      m_device, nvh::loadFile("spv/lidar_points.comp.spv", true, defaultSearchPaths, true), VK_SHADER_STAGE_COMPUTE_BIT);
  if(vkCreateComputePipelines(m_device, VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &m_pipeline) != VK_SUCCESS)
  {
    ...
    throw std::runtime_error("Failed to create LiDAR point generation compute pipeline");
  }
  vkDestroyShaderModule(m_device, pipelineInfo.stage.module, nullptr);
}

void LidarPointGenerationPipeline::updateDescriptorSet(const TlasDescriptorInfo& tlasInfo,
                                                       VkBuffer sensorBuffer,
                                                       VkBuffer pointBuffer)
{
