...
void PreviewPipeline::setup(VkDevice device,
...
void PreviewPipeline::destroyGraphicsPipeline()
{
  destroyFramebuffer();
  if(_device != VK_NULL_HANDLE)
  {
...
void PreviewPipeline::recreateAovTargets(VkExtent2D size)
{
...
void PreviewPipeline::createGraphicsPipeline(VkDescriptorSetLayout sceneDescriptorSetLayout)
{
  destroyGraphicsPipeline();
  ...
  pipelineGenerator.addShader(nvh::loadFile("spv/mesh.vert.spv", true, defaultSearchPaths, true),
                              VK_SHADER_STAGE_VERTEX_BIT);
  pipelineGenerator.addShader(nvh::loadFile("spv/mesh.frag.spv", true, defaultSearchPaths, true),
...
std::optional<ExportedAovTexture> PreviewPipeline::getAovTexture(Aov aov) const
{
...
void PreviewPipeline::createOffscreenImage(nvvk::Texture& texture,
                                          VkFormat format,
...
void PreviewPipeline::createFramebuffer()
{
...
void PreviewPipeline::destroyFramebuffer()
{
