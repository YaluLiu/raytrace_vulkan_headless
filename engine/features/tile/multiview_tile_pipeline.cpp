...
void MultiviewTilePipeline::setup(VkDevice device,
...
void MultiviewTilePipeline::destroy()
{
...
void MultiviewTilePipeline::destroyGraphicsPipeline()
{
...
bool MultiviewTilePipeline::ensureResources(VkDescriptorSetLayout sceneDescriptorSetLayout,
                                                  const PreviewPipeline& referencePipeline,
                                                  uint32_t viewCount)
{
  ...
  const bool matches = _renderPass != VK_NULL_HANDLE && _graphicsPipeline != VK_NULL_HANDLE
  ...
  return _renderPass != VK_NULL_HANDLE && _graphicsPipeline != VK_NULL_HANDLE
         && _domeBackgroundPipeline != VK_NULL_HANDLE && _pipelineLayout != VK_NULL_HANDLE;
}

void MultiviewTilePipeline::createRenderPass()
{
...
void MultiviewTilePipeline::createGraphicsPipeline()
{
  ...
  pipelineState.addBindingDescription(
      nvvk::GraphicsPipelineState::makeVertexInputBinding(0, sizeof(MeshVertex), VK_VERTEX_INPUT_RATE_VERTEX));
  pipelineState.addAttributeDescription(
      nvvk::GraphicsPipelineState::makeVertexInputAttribute(0, 0, VK_FORMAT_R32G32B32_SFLOAT, offsetof(MeshVertex, pos)));
  pipelineState.addAttributeDescription(
      nvvk::GraphicsPipelineState::makeVertexInputAttribute(1, 0, VK_FORMAT_R32G32B32_SFLOAT, offsetof(MeshVertex, nrm)));
  ...
  pipelineState.addAttributeDescription(nvvk::GraphicsPipelineState::makeVertexInputAttribute(
      3, 0, VK_FORMAT_R32G32_SFLOAT, offsetof(MeshVertex, texCoord)));
  pipelineState.addAttributeDescription(nvvk::GraphicsPipelineState::makeVertexInputAttribute(
      4, 0, VK_FORMAT_R32G32B32A32_SFLOAT, offsetof(MeshVertex, tangent)));
  ...
  _graphicsPipeline = pipelineGenerator.createPipeline();
  if(_graphicsPipeline == VK_NULL_HANDLE)
  {
    throw std::runtime_error("Failed to create tile multiview graphics pipeline");
  }
...
bool MultiviewTilePipeline::draw(const VkCommandBuffer& cmdBuf,
                                            ...
                                            std::span<const int> instanceIds)
{
  ...
  if(_debug != nullptr)
  {
    _debug->beginLabel(cmdBuf, "TileMultiviewDraw");
  }
