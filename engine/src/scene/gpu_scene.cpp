...
void GpuScene::uploadTextureResources(const VkCommandBuffer& cmdBuf,
...
void GpuScene::updateMeshGeometry(uint32_t meshId, const MeshGeometry& geometry)
{
  ...
  std::vector<MeshVertex>& now_vertices = m_meshGeometries[meshId].vertices;
  std::vector<uint32_t>& now_indices = m_meshGeometries[meshId].indices;
  MeshBuffers& model = m_objModel[meshId];
...
void GpuScene::updateMaterialsAtRuntime(const std::vector<MaterialUpdate>& updates)
{
  ...
  {
    VkDeviceSize offset = upd.materialIndex * sizeof(Material);
    vkCmdUpdateBuffer(cmdBuf, m_objModel[upd.modelIndex].matColorBuffer.buffer, offset, sizeof(Material),
                      &upd.newMaterial);
  }
...
void GpuScene::destroyMeshBuffers()
{
  if(m_alloc != nullptr)
  {
