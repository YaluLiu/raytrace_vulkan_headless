...
public:
  ...
  bool updateInstance(uint32_t rendererInstanceIndex, glm::mat4 transform, bool visible, uint32_t traceMask);
  ...
  std::span<const int> externalInstanceIds() const { return m_externalInstanceIds; }

private:
  std::vector<SceneInstance> m_instances;
  std::vector<int> m_externalInstanceIds;
};
