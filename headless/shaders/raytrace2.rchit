...
vec3 computeDirectLighting(vec3 worldPos, vec3 worldNrm, WaveFrontMaterial mat, vec3 textureColor)
{
  ...
  for(int i = 0; i < numLights; i++)
  {
    ...
    vec3  origin     = worldPos + worldNrm * shadowBias;
...
void main()
{
  ...
  worldNrm = faceforward(worldNrm, gl_WorldRayDirectionEXT, worldNrm);
  ...
  vec3 direct   = computeDirectLighting(worldPos, worldNrm, mat, textureColor);
