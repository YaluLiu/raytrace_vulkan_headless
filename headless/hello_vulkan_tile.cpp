...
namespace {
std::string joinPath(const std::filesystem::path &directory,
                     const char *filename) {
...
void HelloVulkan::renderTileAtlasMultiview(const VkCommandBuffer &cmdBuf) {
  ...
  const uint32_t cameraCount =
      static_cast<uint32_t>(std::min<size_t>(m_cameras.size(), capacity));
  const uint32_t effectiveViewCount =
      getTileMultiviewEffectiveViewCount(cameraCount, capacity);
  ...
  const VkExtent2D tileExtent = config.tileExtent();
  try {
    if (!m_tileMultiviewRasterPipeline.ensureResources(
            m_descSetLayout, m_tileRasterPipeline, effectiveViewCount)) {
    ...
    if (!m_tileLayerOutput.ensureResources(
            config, m_tileRasterPipeline,
            m_tileMultiviewRasterPipeline.getRenderPass(),
            effectiveViewCount)) {
      fallbackOrSkip("failed to create layered tile resources");
      return;
    }
  } catch (const std::exception &e) {
    std::cerr << "[HelloVulkan] Tile multiview setup failed: " << e.what()
              << std::endl;
    if (m_tileRenderMode == HeadlessTileRenderMode::MultiviewPreferred) {
      renderTileAtlas(cmdBuf);
    }
  ...
  for (uint32_t firstCameraIndex = 0; firstCameraIndex < cameraCount;
       firstCameraIndex += effectiveViewCount) {
    const uint32_t batchCameraCount =
        std::min(effectiveViewCount, cameraCount - firstCameraIndex);
    updateTileFrameUniformBufferForBatch(cmdBuf, firstCameraIndex,
                                         batchCameraCount, effectiveViewCount,
                                         tileExtent);
    const bool rendered = m_tileMultiviewRasterPipeline.rasterize(
        cmdBuf, m_tileLayerOutput.getFramebuffer(), tileExtent, m_descSet,
        m_objModel, m_instances, m_instanceIds);
    const bool copied =
        rendered && m_tileAtlas.copyLayersFrom(cmdBuf, m_tileLayerOutput,
                                               firstCameraIndex,
                                               batchCameraCount);
    if (!copied) {
...
void HelloVulkan::saveTileAtlasOutputs(const std::string &outputDirectory) {
  ...
  auto readFloatImage = [&](const nvvk::Texture &texture,
                            uint32_t componentCount) {
    const VkDeviceSize imageSize =
        static_cast<VkDeviceSize>(pixelCount) * componentCount * sizeof(float);
    std::vector<float> result(static_cast<size_t>(pixelCount) * componentCount,
                              0.0f);
    ...
    VkImageMemoryBarrier toGeneral = makeColorImageBarrier(
        texture.image, VK_ACCESS_TRANSFER_READ_BIT,
        VK_ACCESS_TRANSFER_WRITE_BIT | VK_ACCESS_SHADER_WRITE_BIT |
            VK_ACCESS_SHADER_READ_BIT,
  ...
  const std::vector<float> colorFloats =
      readFloatImage(m_tileAtlas.getColorTexture(), 4);
  std::vector<uint8_t> colorBytes(static_cast<size_t>(pixelCount) * 4, 255);
  for (uint32_t pixel = 0; pixel < pixelCount; ++pixel) {
    colorBytes[pixel * 4 + 0] = static_cast<uint8_t>(
        glm::clamp(colorFloats[pixel * 4 + 0], 0.0f, 1.0f) * 255.0f);
    colorBytes[pixel * 4 + 1] = static_cast<uint8_t>(
        glm::clamp(colorFloats[pixel * 4 + 1], 0.0f, 1.0f) * 255.0f);
    colorBytes[pixel * 4 + 2] = static_cast<uint8_t>(
        glm::clamp(colorFloats[pixel * 4 + 2], 0.0f, 1.0f) * 255.0f);
    colorBytes[pixel * 4 + 3] = static_cast<uint8_t>(
        glm::clamp(colorFloats[pixel * 4 + 3], 0.0f, 1.0f) * 255.0f);
  }

  const std::string colorPath = joinPath(outputDir, "tile_color_atlas.png");
  if (stbi_write_png(colorPath.c_str(), static_cast<int>(extent.width),
                     static_cast<int>(extent.height), 4, colorBytes.data(),
                     static_cast<int>(extent.width * 4)) == 0) {
    throw std::runtime_error("Failed to write tile color atlas: " + colorPath);
  }

  const std::vector<float> depthFloats =
      readFloatImage(m_tileAtlas.getDepthTexture(), 1);
  ...
  depthFile.write(
      reinterpret_cast<const char *>(depthFloats.data()),
  ...
  if (!metadata) {
    throw std::runtime_error("Failed to open tile depth metadata: " +
                             metadataPath);
  }
  metadata << "{\n"
           << "  \"width\": " << extent.width << ",\n"
